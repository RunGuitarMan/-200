...
```
HTTP server on port 8080 [kqueue] workers=1 body=2B
```

### Опции командной строки

```bash
./server -p PORT       # Порт (по умолчанию: 8080)
./server -w WORKERS    # Количество worker-процессов (по умолчанию: 1)
./server -s SIZE       # Размер тела ответа в байтах (по умолчанию: 2)
```
...
Сервер поддерживает мультипроцессный режим через опцию `-w`:

```bash
# Запуск с 4 воркерами
./server -w 4

# Запуск с количеством воркеров = количеству ядер
./server -w $(sysctl -n hw.ncpu)
```

Сервер выводит RPS-счётчик для каждого воркера в stderr.

### Остановка сервера

При запуске с `-w` создаётся файл `server.pid`. Для корректной остановки:
...
Сервер выполняет graceful shutdown: посылает SIGTERM всем воркерам и ждёт их завершения.
...
# Запуск сервера с воркерами
echo "Запуск сервера с $CORES воркерами..."
./server -w $CORES
...
Используйте `stop.sh` — скрипт сначала пытается прочитать PID-файл `server.pid`, а при его отсутствии использует `pkill -x server`:
